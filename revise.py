...
# 원문텍스트 출력
def sentence_print(node_list, recipe, sequence_list):

    direction_recipe = ""
    is_dir = False
    for element in recipe:
        if is_dir == True:
            direction_recipe += element
        if element == '[조리방법]\n' and is_dir == False:
            is_dir = True

    line_recipe = direction_recipe.split("\n")
    line = 0
    
    for dict in sequence_list:
        ...
        for node in node_list:
            for w_ele in node['word']:
                ...
                if start_id <= begin and end <= end_id and text in line_recipe[line] and line < len(line_recipe):
                    dict['sentence'] = line_recipe[line]
                else:
                    line += 1
                    break
    ...
    for m_ele in node['morp']:
        if m_ele['type'] == 'VV':
            ...
            if act in cooking_act_dict:
                # 6가지 요소
                # 이걸 line에 넣을 것
                seq_dict = {'cond' : cond, 'act': act, 'tool': [], 'ingre': [], 'seasoning': [], 'volume': [],
