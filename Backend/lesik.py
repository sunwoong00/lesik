...
def version2():
    ...
    if version2send.checkpoint == 1:
        ...
        return render_template("microrecipe.html", recipe=version2send.recorded_recipe_v2)
    else:
        return render_template("microrecipe.html", recipe="\n".join(version2send.recorded_recipe_v2))
...
@app.route('/microrecipe/returnjson', methods=["POST", "GET"])
def root():
    original_recipe = None
    if request.method == 'POST':
