...
def classify(seq_list):
    slice = ["썰다", "슬라이스", "다이스", "가르다", "다지다"]
    pre_process = ["밑간하다", "재우다", "숙성시키다", "불리다", "버무리다", "절이다"]
    use_fire = ["끓이다", "끄다", "켜다", "가열하다", "볶다", "끓어오르다", "가열하다", "예열하다", "굽다", "삶다", "조리다", "졸이다", "데치다", "찌다", "튀기다", "지지다", "부치다", "익히다", "데우다"]
    polysemy = ["넣다", "첨가하다", "담다"]
    single = ["접다", "닦다", "털다", "건지다", "바르다", "해감하다", "식다", "우리다", "식히다", "무치다", "곁들이다", "벗기다", "따라내다", "덮다", "묻히다", "빚는다", "갈다", "쑤다", "얹다", "올리다", "끼얹다", "제거하다", "씻다", "걷어내다", "걷다", "찌르다", "풀다", "뜨다", "두르다", "담그다", "찢다", "하다", "거르다", "젓다", "내다", "섞다", "붓다", "헹구다", "뿌리다", "빼다", "버리다", "갈다", "따르다"]
    useless = ["준비하다", "조절하다", "만들다"]
    middle_class=[]
    top_class_slice=[]
    top_class_preProcess=[]
    top_class_polysemy=[]
    top_class_useFire=[]
    top_class_single=[]
    top_class_useless=[]
    ...
    for sequence in seq_list:
        if sequence['act'] in slice:
            top_class_slice.append(sequence['act'])
            middle_class.append(sequence['act'])
            sequence['act'] = sequence['act']+"(대분류:slice)"
        elif sequence['act'] in pre_process:
            top_class_preProcess.append(sequence['act'])
            middle_class.append(sequence['act'])
            sequence['act'] = sequence['act']+"(대분류:pre_process)"
        elif sequence['act'] in use_fire:
            top_class_useFire.append(sequence['act'])
            middle_class.append(sequence['act'])
            sequence['act'] = sequence['act']+"(대분류:use_fire)"
        elif sequence['act'] in polysemy:
            top_class_polysemy.append(sequence['act'])
            middle_class.append(sequence['act'])
            sequence['act'] = sequence['act']+"(대분류:polysemy)"
        elif sequence['act'] in useless:
            top_class_useless.append(sequence['act'])
            middle_class.append(sequence['act'])
            sequence['act'] = sequence['act']+"(대분류:useless)"
        else:
            top_class_single.append(sequence['act'])
            middle_class.append(sequence['act'])
            sequence['act'] = sequence['act']+"(대분류:single)"
        ...
        if m_ele['type'] == 'VV':
            ...
            if act in cooking_act_dict:
                ...
                seq_dict = {'cond': "", 'act': act, 'tool': [], 'ingre': [], 'seasoning': [], 'volume': [],
                            'zone': "", "start_id": prev_seq_id + 1, "end_id": act_id, "sentence": "", "standard":""}
